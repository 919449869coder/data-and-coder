...
for img_name in img_names:
    ...
    markers,ret=watershed_show_obj(GrayImage,img,color=250)
    markers_nir,ret_nir=watershed_show_obj(GrayImage_nir,img_nir,color=60)  ###
    
    height,width=get_img_size(markers_nir)
    area=get_all_class_coordinate(ret_nir,height,width,markers_nir)
    ...
    for rect in rects:
        ...
        oneimg_nir[markers_nir != m] = [0,0,0]      #判断像素点值不是标签值得变为o
